      ...
      const tryLoad = (src: string, isFallback = false) => {
        ...
        script.onload = () => {
          if (!isFhevmWindowType(window, this._trace)) {
            ...
            reject(
              new Error(
                `RelayerSDKLoader: Relayer SDK script has been successfully loaded from ${src}, however, the window.relayerSDK object is invalid.`
              )
            );
            return;
          }
        ...
        script.onerror = () => {
          console.log(`[RelayerSDKLoader] script onerror for ${src}`);
          if (!isFallback) {
            console.log("[RelayerSDKLoader] retry with local SDK...");
            tryLoad(SDK_LOCAL_URL, true);
          } else {
            reject(
              new Error(
                `RelayerSDKLoader: Failed to load Relayer SDK from ${SDK_CDN_URL} and local fallback ${SDK_LOCAL_URL}`
              )
            );
          }
        };
...
function isFhevmRelayerSDKType(
  ...
  if (!hasZamaEth && !hasSepolia) {
    trace?.(
      "RelayerSDKLoader: relayerSDK must expose either ZamaEthereumConfig or SepoliaConfig"
    );
    return false;
  }
